...
import { LctStatistics, ViolationItem } from '../types/report';
...
interface LctSectionEditorProps {
  stats: LctStatistics;
  violations: ViolationItem[];
  onStatsChange: (stats: LctStatistics) => void;
  onViolationsChange: (violations: ViolationItem[]) => void;
  disabled?: boolean;
}

export const LctSectionEditor: React.FC<LctSectionEditorProps> = ({
  stats,
  violations,
  onStatsChange,
  onViolationsChange,
  ...
  const percentages = calculateLctPercentages(stats);

  const handleStatChange = (field: keyof LctStatistics, value: any) => {
    if (disabled) return;
    let sanitized = value;
    if (typeof value === 'number') {
      sanitized = Math.max(0, isNaN(value) ? 0 : value);
    }
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-slate-800">
            II. Việc Thực Hiện Lệnh Công Tác (LCT)
          </h2>
        </div>
      </div>
      ...
      <div className="mb-6">
        <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wide mb-3">
          1. Số liệu tổng hợp LCT (Tự động tính phần trăm)
        </h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          ...
          <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
            ...
            <input
              ...
              placeholder="/"
              className="w-full text-sm font-medium text-slate-800 bg-white px-2.5 py-2 rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60"
            />
            <div className="mt-1 text-[11px] text-slate-400">
              Xuất hiện ở cột cuối bảng tổng kết
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wide">
            2. Danh sách chi tiết LCT có nội dung không phù hợp ({violations.length} mục)
          </h3>
        </div>
        ...
        {violations.length === 0 ? (
          <div className="text-center py-6 border-2 border-dashed border-slate-200 rounded-xl bg-slate-50/50">
            ...
            <p className="text-sm font-semibold text-slate-700">
              Không có LCT nào có nội dung không phù hợp
            </p>
                  ...
                  <div className="md:col-span-4">
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      >
                        ...
                        {LCT_VIOLATION_OPTIONS.map((opt, i) => (
